...
object Service extends IOApp:
  ...
  private def appResource[F[_]: Async]: Resource[F, Server] =
    for
      conf <- Resource.eval(Async[F].fromEither(AppConf.parse))
      db <- DoobieDatabase.default[F](conf)
      service = Service[F](DemoDatabase(db))
      server <- serverResource(service)
    yield server

  private def serverResource[F[_]: Async](service: Service[F]) = EmberServerBuilder
    .default[F]
...
class Service[F[_]: Async](db: DemoDatabase[F]) extends Implicits[F]:
  private val noCache = `Cache-Control`(`no-cache`(), `no-store`, `must-revalidate`)
